...
Each cell is identified by ``(snr_idx, n_idx, type_idx, run_idx)``; seeds are
derived deterministically by :class:`CachedEvalDataset`, so the manifest
itself stays tiny (a few KB) regardless of how many Monte Carlo runs are
described.

    python -m scripts.generate_eval_manifest --out eval_manifest.json

The default ``--sequence-types`` includes positive_uav, empty_background,
hard_negative, and mixed_uav_and_distractors so the eval covers both
probability-of-detection (positive cells) and false-alarm-rate (target-absent
cells) at every (SNR, N).
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--sequence-types",
        nargs="+",
        default=list(DEFAULT_SEQUENCE_TYPES),
        help="Sequence types to enumerate (positive_uav / empty_background / hard_negative / mixed_uav_and_distractors).",
    )
    ...
    for key in ("snr_db", "n_frames", "seed", "sequence_type"):
    ...
    manifest = {
        ...
        "sequence_types": list(args.sequence_types),
        "monte_carlo_runs_per_cell": args.mc_runs,
        "seed_strategy": "blake2b(b'drone-eval', f'{snr_idx}-{n_idx}-{run_idx}[-{type_idx if >0}]')[:4]",
    ...
    total = len(snr_grid) * len(args.n_grid) * len(args.sequence_types) * args.mc_runs
    print(
        f"Wrote {args.out} | {len(snr_grid)} SNR x {len(args.n_grid)} N x "
        f"{len(args.sequence_types)} types x {args.mc_runs} MC = {total} sequences."
    )
