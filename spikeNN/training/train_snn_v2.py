"""Train SpikingTrackletUNet V2 with capacity parity + continuous-membrane readout.

V2 vs V1 differences (only — everything else identical):
  - base_channels:    16  -> 24   (~150K params vs ~64K, matches baseline)
  - bottleneck_chan:  32  -> 48
  - readout_mode:     "spike_rate" -> "membrane"

The continuous-membrane readout preserves amplitude info at the output while
keeping spiking dynamics inside the network. This is the principled fix for
the matched-filter information loss diagnosed in the V1 run.
"""
...
# V2-specific output paths (don't clobber V1).
RESULTS_DIR = ROOT / "spikeNN" / "results"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)
SNN_V2_CHECKPOINT = RESULTS_DIR / "snn_v2_checkpoint.pt"
SNN_V2_HISTORY = RESULTS_DIR / "snn_v2_history.json"
SNN_V2_SNR_SWEEP = RESULTS_DIR / "snn_v2_snr_sweep.json"
SNN_V2_STAGE_CKPT = lambda stage: RESULTS_DIR / f"snn_v2_stage{stage}_checkpoint.pt"


def main():
    print(f"=== SpikingTrackletUNet V2 training ===", flush=True)
    print(f"  V2 diffs: base_channels 16->24, bottleneck 32->48, "
          f"readout_mode spike_rate->membrane", flush=True)
    ...
    t0 = time.time()
    s1_train_specs = td.make_video_specs(v1.STAGE1["n_train"], seed=1001,
                                          snr_range_db=v1.STAGE1["snr"])
    s1_val_specs = td.make_video_specs(v1.STAGE1["n_val"], seed=2002,
                                        snr_range_db=v1.STAGE1["snr"])
    s2_train_specs = td.make_video_specs(v1.STAGE2["n_train"], seed=3003,
                                          snr_range_db=v1.STAGE2["snr"])
    s2_val_specs = td.make_video_specs(v1.STAGE2["n_val"], seed=4004,
                                        snr_range_db=v1.STAGE2["snr"])
    s3_train_specs = td.make_video_specs(v1.STAGE3["n_train"], seed=5005,
                                          snr_range_db=v1.STAGE3["snr"])
    s3_val_specs = td.make_video_specs(v1.STAGE3["n_val"], seed=6006,
                                        snr_range_db=v1.STAGE3["snr"])
    ...
    snn_cfg = SpikingTrackletUNetConfig(
        ...
        base_channels=24,           # V2: 2× the V1 width (16 -> 24/48 stack)
        bottleneck_channels=48,
        crop_size=v1.CROP_SIZE,
        readout_mode="membrane",     # V2: continuous-membrane readout
    )
    model = SpikingTrackletUNet(snn_cfg).to(device)
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"SNN V2 model: {n_params:,} trainable parameters", flush=True)
    ...
    h1, best1 = v1.run_stage(model, loss_fn, opt, s1_train, s1_val,
                              v1.STAGE1["epochs"], device, "warmup", 0, rng,
                              v1.STAGE1["batches"])
    all_history.extend(h1)
    stage_results["stage1_best_val_auc"] = best1
    torch.save(model.state_dict(), SNN_V2_STAGE_CKPT(1))
    ...
    h2, best2 = v1.run_stage(model, loss_fn, opt, s2_train, s2_val,
                              v1.STAGE2["epochs"], device, "transit",
                              v1.STAGE1["epochs"], rng, v1.STAGE2["batches"])
    all_history.extend(h2)
    stage_results["stage2_best_val_auc"] = best2
    torch.save(model.state_dict(), SNN_V2_STAGE_CKPT(2))

    # --- Stage 3 (with rehearsal) -----------------------------------------
    ...
    h3, best3 = v1.run_stage(model, loss_fn, opt, s3_train_with_rehearsal, s3_val,
                              ...
                              v1.STAGE1["epochs"] + v1.STAGE2["epochs"], rng,
                              v1.STAGE3["batches"])
    all_history.extend(h3)
    stage_results["stage3_best_val_auc"] = best3

    torch.save(model.state_dict(), SNN_V2_CHECKPOINT)
    with open(SNN_V2_HISTORY, "w") as f:
        json.dump({"history": all_history, "stage_results": stage_results,
                   "param_count": n_params,
                   "config": {"base_channels": 24, "bottleneck_channels": 48,
                              "readout_mode": "membrane"}}, f, indent=2)
    print(f"\nCheckpoint saved to {SNN_V2_CHECKPOINT}", flush=True)
    print(f"Per-stage best val ROC-AUC: stage1={best1:.3f}  stage2={best2:.3f}  stage3={best3:.3f}",
          flush=True)

    # --- SNR sweep ---------------------------------------------------------
    print(f"\n=== SNR sweep on SNN V2 ({len(v1.SNR_GRID)} cells, "
          f"{v1.SNR_RUNS_PER_CELL} runs/cell) ===", flush=True)
    snr_rows = td.snr_sweep(model, s3_ds_cfg, v1.SNR_GRID,
                            runs_per_cell=v1.SNR_RUNS_PER_CELL, device=device)
    with open(SNN_V2_SNR_SWEEP, "w") as f:
        json.dump(snr_rows, f, indent=2)
    print(f"SNR sweep saved to {SNN_V2_SNR_SWEEP}", flush=True)
    for r in snr_rows:
        print(f"  SNR={r['snr_db']:+5.1f} dB | Pd@FAR=1/100 = "
              f"{r['pd_at_far_1_per_100']:.2f}  "
